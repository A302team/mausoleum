...
public:
    ...
    void run(int port)
    {
        uWS::App().ws<int>("/*", {.open = [this](auto *ws)
                                  { std::cout << "[Lobby] 클라이언트 접속!" << std::endl; },
...
private:
    ...
    {
        handlers["create_room"] = [this](auto *ws, const json &data)
        { handleCreateRoom(ws, data); };
        handlers["join_room"] = [this](auto *ws, const json &data)
        { handleJoinRoom(ws, data); };
        handlers["ready"] = [this](auto *ws, const json &data)
        { handleReady(ws, data); };
        handlers["start_game"] = [this](auto *ws, const json &data)
        { handleStartGame(ws, data); };
        handlers["get_room_list"] = [this](auto *ws, const json &data)
        { handleGetRoomList(ws, data); };
        handlers["check_nickname"] = [this](auto *ws, const json &data)
        { handleCheckNickname(ws, data); };
        handlers["leave_room"] = [this](auto *ws, const json &data)
        { handleLeaveRoom(ws, data); };
        handlers["chat_message"] = [this](auto *ws, const json &data)
