...
UCLASS()
class A302SHARED_API AMyCharacter : public ACharacter, public IA302CharacterBridge
{
	GENERATED_BODY()
...
public:
	...
	void NotifyKilledCharacter() override;
	void NotifyTimedKnifeAttackSucceeded() override;
	void RegisterTimedKillEvent(UObject* EventInstance) override;
	void ClearTimedKillEvent(UObject* EventInstance) override;
	void ForceDeathByPersonalEvent() override;
	void SetTimedKnifeAttackInProgress(bool bInProgress) override;
	void EquipKnifeWeapon() override;
	void EquipTimeKnifeWeapon() override;
	void EquipShieldWeapon() override;
	void PlayShieldBlockPresentation() override;
	void SetQTEInputMode(bool bIsQTE) override;
	void BroadcastPublicMaliceAnnouncement(const FString& PlayerName, int32 MaliceCount) override;
