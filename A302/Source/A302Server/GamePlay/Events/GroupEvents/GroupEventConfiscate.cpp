...
void UGroupEventConfiscate::ExecuteEvent_Implementation(ACharacter* InstigatorCharacter)
{
	...
	for (FConstPlayerControllerIterator It = GetWorld()->GetPlayerControllerIterator(); It; ++It)
	{
		...
		IA302ClientEventBridge* ClientEventBridge = Cast<IA302ClientEventBridge>(PlayerController);
	...
	{
		if (IA302ClientEventBridge* ClientEventBridge = Cast<IA302ClientEventBridge>(PlayerController))
		{
...
void UGroupEventConfiscate::ResolveVote()
{
	...
	if (IA302ClientEventBridge* ClientEventBridge = Cast<IA302ClientEventBridge>(TargetPlayerController))
	{
...
void UGroupEventConfiscate::FinishEvent(const FText& ResultText)
{
	for (APlayerController* Participant : Participants)
	{
		if (IA302ClientEventBridge* ClientEventBridge = Cast<IA302ClientEventBridge>(Participant))
		{
